...
if ndims(I) == 3
     for i=1:3
        I2(:,:,i) = imdistcorrect(I(:,:,i),k,ex);
     end   
elseif ismatrix(I)
    I2 = imdistcorrect(I,k,ex);
else
...
% Nested function that perfoms the transformation
    function I3 = imdistcorrect(I,k,ex)
    ...
    s = distortfun(r,k,ex,p.Results.ftype);
    % un-normalize s
    s2 = s * R;
...
% Nested function that pics the model type to be used
    function s = distortfun(r,k,ex,fcnum)
        switch fcnum
            case(1)
                s = r.*(1./(1+k.*r));
            case(2)

function fret_correct_nonlin(aexp,dexp,fexp,abt,dbt,imin,FRETeff,leave_neg,folder,varargin)
...
for i = 1:length(df)
    ...
    if length(abt) == 1
        ...
        sabt = load(abt);
        sdbt = load(dbt);
        abt = zeros(size(axam,1),size(axam,2));
        dbt = abt;
        w = find(axam > 0 & dxdm > 0 & dxam > 0);
        ai = axam(w);
        di = dxdm(w);
        
        abt(w) = spline_intrp(sabt,ai);
        dbt(w) = spline_intrp(sdbt,di);
        
        fc = dxam-abt.*axam-dbt.*dxdm;
    end

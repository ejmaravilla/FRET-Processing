...
if strcmpi(SaveParams.correct,'y') && isempty(file_search('cna_\w+.TIF',folder));
    for i = 1:length(SaveParams.exp_cell)
        fret_correct([prefix SaveParams.exp_cell{i} '.*' SaveParams.Achannel '.*.TIF'],...
            [prefix SaveParams.exp_cell{i} '.*' SaveParams.Dchannel '.*.TIF'],...
            [prefix SaveParams.exp_cell{i} '.*' SaveParams.FRETchannel '.*.TIF'],...

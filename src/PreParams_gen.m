...
% channels = {'Teal' 'Venus' 'FWCy5' 'FWTR' 'FWFITC' 'FWDAPI'};
channels = {'TVFRET' 'Venus' 'Teal' 'FWCy5' 'FWDAPI' 'FWFITC' 'FWTR'};
...
if isempty(file_search('\w+reg_params.txt',folder));
...
if isempty(file_search('dark_mean\w+.TIF',folder));
    for i = 1:num_channels
        ...
        %Need to crop to make the correct size
        sz = size(dark_mean{i});
        crop = [round(0.0246*sz(1)) round(0.0246*sz(1)) round(0.9509*sz(1)) round(0.9509*sz(1))];
        dark_mean{i} = imcrop(dark_mean{i},crop);
...
rehash
if isempty(file_search('shade_smth\w+.TIF',folder));
    for i = 1:num_channels

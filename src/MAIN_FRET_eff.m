...
if ~isempty(file_search('\w+.lsm',folder))
    for i = 1:SaveParams.num_exp
        lsm2tif1([SaveParams.exp_cell{i} '\w+.lsm'],folder,{SaveParams.Achannel, SaveParams.FRETchannel,SaveParams.Dchannel});
    end
end
...
if strcmpi(SaveParams.correct,'y') && isempty(file_search('cna_\w+.TIF',folder));
    for i = 1:length(SaveParams.exp_cell)
        fret_correct_efficiency([prefix SaveParams.exp_cell{i} '\w+\d+\w+' SaveParams.Achannel '.TIF'],...
...
%% Calculate forces from Grashoff Nature 2010 force calibration curve and eff_ images
rehash
if isempty(file_search('force_\w+.TIF',folder))
    fret_force_calibration(['eff_\w+' SaveParams.FRETchannel '.TIF'],SaveParams)
end
    ...
    for i = 1:SaveParams.num_exp
        ...
        if length(file_search('blb_anl\w+.txt',folder)) < i
            blob_analyze_centroids(...
                ...
                ['force_' prefix SaveParams.exp_cell{i} '\w+' SaveParams.FRETchannel '.TIF'],...
...
% With extra FRET calculations, num_channels is now 6
SaveParams.num_channel = 6;
save(fullfile(pwd,folder, ['SaveParams_' folder '.mat']),'-struct','SaveParams');

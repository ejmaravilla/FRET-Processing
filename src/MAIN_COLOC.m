...
if ~isempty(file_search('\w+.lsm',folder))
    for i = 1:SaveParams.num_exp
        lsm2tif1([SaveParams.exp_cell{i} '\w+.lsm'],folder,{SaveParams.channel1, SaveParams.channel2});
    end
end
...
rehash
if isempty(file_search('bslocal\w+.TIF',folder))
for i = 1:SaveParams.num_exp
    local_bs(...
...
if strcmpi(SaveParams.reg_select,'y')
    for i = 1:SaveParams.num_exp
        newcols = boundary_dist(...
            ['bslocal2_' prefix SaveParams.exp_cell{i} '\w+\d+\w+' SaveParams.channel1 '.TIF'],...
        ...
        rehash
        img_names = file_search(['bslocal2_' prefix SaveParams.exp_cell{i} '\w+\d+\w+' SaveParams.channel1 '.TIF'],folder);
        num_img = length(img_names);
        for j = 1:num_img
            mask_img(['polymask_cell\d+\w+' img_names{j}(1:end-4) '.png'],folder)
        end
